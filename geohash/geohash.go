...
func (this *Box) Width() (width float64) {
	width = this.MaxLng - this.MinLng
	return
}

//计算高度
func (this *Box) Height() (height float64) {
	height = this.MaxLat - this.MinLat
	return
}
...
func Encode(lat, lng float64, precision int) (geohashCode string) {
	var (
		...
		maxLat    float64 = 90                //初始化最大纬度
		minLat    float64 = -90               //初始化最小纬度
		maxLng    float64 = 180               //初始化最大经度
		minLng    float64 = -180              //初始化最小经度
		mid       float64 = 0                 //中间值标记
		bit               = 0                 //二进制个数标记
	...
	for preLen < precision {
		...
		if bit < 4 {
			...
			num := gotil.BinaryToDecimal(bitNum)
